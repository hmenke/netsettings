...
__git_ps1_show_upstream ()
{
	...
	svn*)
		...
		local svn_upstream="$(
			__git_with_timeout log --first-parent -1 \
	...
	if [ -z "$legacy" ]; then
		...
		local commits
		if commits="$(__git_with_timeout rev-list --left-right "$upstream_type"...HEAD 2>/dev/null)"
		then
...
__git_ps1 ()
{
		...
		if [ -n "$b" ]; then
			...
			files)
				...
				*)
				...
				;;
			*)
				head="$(__git_with_timeout symbolic-ref HEAD 2>/dev/null)"
				;;
			esac
	...
	if [ "${GIT_PS1_SHOWCONFLICTSTATE-}" = "yes" ] &&
	   [ "$(__git_with_timeout ls-files --unmerged 2>/dev/null)" ]; then
		conflict="|CONFLICT"
	fi
